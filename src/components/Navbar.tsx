...
const Navbar = () => {
  ...
  return (
        <>
        <nav className={"w-full z-10 p-2 py-3 md:px-[20%] bg-theme-b backdrop-filter backdrop-blur-sm fixed top-0 left-0 shadow-xl " + (isNavOpen ? " flex sm:flex-row flex-col justify-evenly items-center sm:h-fit h-screen " : " hidden h-0 ")} onClick={() => setIsNavOpen((isMobileMenu ? false : true))}>
            {
                _navItems.map(({title, url}) => {
                    return <NavItem key={title} title={title} url={url} />
                })
            }
            <Link href={"#contact"} className="bg-theme-w text-theme-b hover:bg-theme-w-alt py-1 px-2 rounded-xl font-bold font-main-alt xl:ml-[25%] transition-all duration-300">Get in Touch</Link>
        </nav>
        {
            isMobileMenu ? 
